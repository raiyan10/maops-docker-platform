...
module validates only the field the *gateway* service actually uses
(`dependency_timeout_seconds`, applied to the gateway's own bounded call
to `app`) - it deliberately does not care about `state_filename` (that's
